...
class FinallySkyCard extends HTMLElement {
  ...
  _fillPopupData(id, container) {
    ...
    if (id === 'energie') {
      ...
      T('ep-ld',(_s('sensor.gx_device_ac_uitgang_dagverbruik')/1000).toFixed(2)+' kWh');
      T('ep-lm',(_s('sensor.gx_device_verbruik_aan_boord_maand')/1000).toFixed(1)+' kWh');
      T('ep-dcv',dcV+' V'); T('ep-dcw',dcW+' W');
      // Kosten & rendement
      const pvM2  = _s('sensor.solar_yield_maand');
      const walM  = _s('sensor.walstroom_verbruik_maand');
      const uitM  = _s('sensor.gx_device_verbruik_aan_boord_maand')/1000;
...
class FinallySkyCardMobile extends HTMLElement {
  ...
  _render() {
    ...
    const loadDag    = (s('sensor.gx_device_ac_uitgang_dagverbruik')/1000).toFixed(2);
    const loadMaand  = (s('sensor.gx_device_quattro_uitgang_maandoverzicht')/1000).toFixed(1);
